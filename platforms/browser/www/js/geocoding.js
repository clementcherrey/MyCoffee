...
function jsonTransform() {
	console.log("in json function -> lol");
	$.getJSON("ajax/WagPregeo.json",function(data) {
		$.each(data,function(key, val) {	
			addressArray.push({id: val.id, brand: val.brand, name: val.name,addresseng: val.addresseng, addresscn: val.addresscn});
		});
		initializeGeo();	
	});
}
...
function codeAddressBaidu(address, tmpId, callback) {
	...
	myGeo.getPoint(address, function(point){
		if (point) {  
			console.log("/" + tmpId + "/" +  brand + "/" +address+ "/" + point.lat + "/" + point.lng);
		callback();	
		}else{
